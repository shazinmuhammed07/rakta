...
async function main() {
    ...
    console.log('Users table result:', users ? 'Found columns: ' + Object.keys(users[0] || {}).join(', ') : 'No data or error');
    if (ue) console.error('Users error:', ue);

    const { data: reqs, error: re } = await supabase.from('requests').select('*').limit(1);
    console.log('Requests table result:', reqs ? 'Found columns: ' + Object.keys(reqs[0] || {}).join(', ' : 'No data or error');
