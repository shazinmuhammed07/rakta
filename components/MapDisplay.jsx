...
const MapDisplayComponent = dynamic(() => import('./MapDisplayComponent'), {
    ssr: false,
    loading: () => (
        <div className="w-full h-[250px] sm:h-[350px] md:h-[400px] bg-neutral-900/50 rounded-xl border border-red-500/20 flex flex-col items-center justify-center animate-pulse glass-panel">
...
export default function MapDisplay({ centerPosition, donorPositions, radius }) {
    ...
    return (
        <div className="h-[250px] sm:h-[350px] md:h-[400px]">
            <MapDisplayComponent centerPosition={centerPosition} donorPositions={donorPositions} radius={radius} />
        </div>
    );
}
