...
import { Loader2, Droplet, User, MapPin, Phone, Activity, AlertCircle, CheckCircle2, Bell } from 'lucide-react';
...
export default function Dashboard() {
    ...
    const myRequests = requests.filter(r => r.requester && r.requester._id === user._id);
    const openExternalRequests = requests.filter(r => r.status === 'pending' && r.requester && r.requester._id !== user._id);
        ...
        <div className="flex-1 bg-gray-50 py-12">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-gray-100 mb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    ...
                    {user.role === 'donor' && (
                        <div className="flex flex-col items-start md:items-end gap-2 bg-gray-50 p-4 rounded-xl border border-gray-100 w-full md:w-auto">
                            <span className="text-sm font-medium text-gray-700">Donation Availability</span>
                            <button
                            ...
                            <span className={`text-xs font-semibold ${user.isAvailable ? 'text-green-600' : 'text-gray-500'}`}>
                                {user.isAvailable ? 'Ready to Donate' : 'Currently Unavailable'}
                            </span>
                        </div>
                    )}
                </div>
                ...
                {user.role === 'requester' ? (
                    <div>
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <Link href="/request" className="bg-red-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-red-700 transition">
                                New Request
                            </Link>
                        </div>
                    ...
                    <div>
                        ...
                        {openExternalRequests.length === 0 ? (
                            <div className="bg-white rounded-2xl p-12 text-center border border-gray-100">
                                ...
                                <p className="mt-1 text-gray-500">There are no pending blood requests at the moment.</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
